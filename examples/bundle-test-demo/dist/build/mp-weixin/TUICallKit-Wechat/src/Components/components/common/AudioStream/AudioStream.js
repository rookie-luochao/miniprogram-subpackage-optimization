...
const s = () => '../OverlayStream/OverlayStream.js',
  i = e.defineComponent({
    ...
    setup(t) {
      const s = t,
        i = l.useViewBackgroundConfig(),
        { callType: n, isGroupCall: v } = e.toRefs(a.useCallInfoContext()),
        { isFloatWindow: d } = e.toRefs(o.useFloatWindowContext()),
        c = e.computed(() => !v.value && !r.TUIGlobal.isPC),
        m = e.computed(() =>
          v.value ? r.TUIGlobal.isPC : n.value === u.CallMediaType.AUDIO
        ),
        f = e.computed(
          () =>
            !v.value && r.TUIGlobal.isPC && n.value === u.CallMediaType.AUDIO
        ...
        C = e.computed(() => i.value[s.userId] || s.avatar),
        I = e.ref(!i.value[s.userId]),
        w = e.ref(i.value[s.userId] ? 'fill' : 'cover');
      e.watch(
        [() => s.userId, i],
        () => {
          i.value[s.userId]
      ...
      return (a, o) => ({
        ...
        b: e.p({
          avatar: a.avatar,
          'bg-image': e.unref(C),
          'show-avatar': e.unref(c),
          username: a.username,
          'show-user-name': e.unref(m),
